...
def test_fit():
    X, y = prepare_data()
    y_answer = np.array([0, 1, 2, 0, 3, 2, 4, 2, 3, 1, 5, 0, 4, 1, 3, 5, 4, 5, 5, 4, 2, 1,
                         1, 0, 3, 3, 0, 4, 2, 5], dtype=np.int32)

    clf = L2ART(max_iter=10, max_class=100, rho=0.2, alpha=1e-5, beta=0.1)
    clf.fit(X)
    print(clf.labels_, y_answer)
    ...
    y_pred2 = clf.predict(X_new)
    y_answer_2 = np.array([4], dtype=np.int32)
    assert np.all(y_pred2 == y_answer_2)
