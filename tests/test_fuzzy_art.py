...
def test_fit():
    X, y = prepare_data()
    y_answer = np.array([0, 1, 3, 0, 2, 3, 3, 3, 2, 1, 4, 0, 3, 1, 2, 4, 3, 4, 4, 3, 3, 1,
                         1, 0, 2, 2, 0, 3, 5, 4], dtype=np.int32)

    clf = FuzzyART(max_iter=3, max_class=10, rho=0.75, alpha=1e-5, beta=0.7)
    clf.fit(X)
    ...
    X_new = np.array([[0.0, 0.9, 1.0, 0.1]])
    y_pred = clf.partial_fit_predict(X_new)
    y_answer_2 = np.array([2], dtype=np.int32)
