...
def test_fit():
    X, y = prepare_data()
    y_answer = np.array([6, 1, 2, 4, 0, 2, 3, 2, 0, 1, 4, 6, 3, 1, 0, 4, 3, 4, 4, 3, 2, 1,
                         1, 6, 0, 0, 6, 3, 5, 4], dtype=np.int32)

    clf = ART2(max_iter=3, max_class=10, a=0.01, b=0.01, c=0.1, d=0.7, theta=0.0, rho=0.96)
    clf.fit(X)
    ...
    X_new = np.array([[0.7, 0.3, 0.3, 0.7]])
    y_pred = clf.partial_fit_predict(X_new)
    y_answer_2 = np.array([3], dtype=np.int32)
